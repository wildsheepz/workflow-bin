...
download() {
    ...
    gh release download "$RELEASE" -R "$REPO" -p '*Linux_x86_64.tar.gz' --dir downloads --clobber
...
verify() {
    ...
    cosign verify-blob \
        ...
        --certificate-identity-regexp 'https://github.com/boostsecurityio/poutine/.*' \
...
extract() {
    ...
    tar -xf setup-$BIN_NAME/*_Linux_x86_64.tar.gz -C ./downloads
    mkdir -p "setup-$BIN_NAME/linux/amd64/bin"
...
store() {
    ...
    rm -fv setup-$BIN_NAME/*_Linux_x86_64.tar.gz

    echo "Storing downloaded bin and checksum files for $BIN_NAME..."
    cp -v downloads/*_Linux_x86_64.tar.gz "setup-$BIN_NAME/"
