...
download() {
    ...
    gh release download "$RELEASE" -R "$REPO" -p '*linux_x64.tar.gz' --dir downloads --clobber
...
extract() {
    ...
    tar -xf setup-$BIN_NAME/*linux_x64.tar.gz -C ./downloads
    mkdir -p "setup-$BIN_NAME/linux/amd64/bin"
...
store() {
    ...
    rm -fv setup-$BIN_NAME/*linux_x64.tar.gz
    echo "Storing downloaded bin and checksum files for $BIN_NAME..."
    cp -v downloads/*linux_x64.tar.gz "setup-$BIN_NAME/"
