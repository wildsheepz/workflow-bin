...
download() {
    ...
    gh release download $RELEASE -R $REPO -p '*linux-amd64.intoto.jsonl' --dir downloads --clobber
...
verify() {
    ...
    set -x
    if [ -f "multi.intoto.jsonl" ]; then
        set -x;
        $BIN_NAME verify-artifact $BIN_NAME-linux-amd64 \
...
extract() {
    ...
    mkdir -p "setup-$BIN_NAME/linux/amd64/bin"
    mv -v "setup-$BIN_NAME/slsa-verifier-linux-amd64" "setup-$BIN_NAME/linux/amd64/bin/$BIN_NAME"
...
store() {
    ...
    rm -fv setup-$BIN_NAME/*linux_amd64

    echo "Storing downloaded bin and checksum files for $BIN_NAME..."
    cp -v downloads/*linux-amd64 "setup-$BIN_NAME/"
