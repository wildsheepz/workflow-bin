...
download() {
    ...
    gh release download "$RELEASE" -R "$REPO" -p 'checksums*' --dir downloads --clobber
...
verify() {
    ...
    set -x
    HASH_INDEX=`xargs < checksums_hashes_order | grep -Eo '^.*SHA-256' | wc -w`
    CHECKSUM=`grep -E 'yq_linux_amd64\s' < checksums | sed 's|  | |g' | cut -d " " -f $((HASH_INDEX+1))`
    echo "$CHECKSUM yq_linux_amd64" > checksums.txt
...
extract() {
    ...
    mkdir -p "setup-$BIN_NAME/linux/amd64/bin"
    mv -v "setup-$BIN_NAME/yq_linux_amd64" "setup-$BIN_NAME/linux/amd64/bin/$BIN_NAME"
...
store() {
    ...
    rm -fv setup-$BIN_NAME/*_linux_amd64

    echo "Storing downloaded bin and checksum files for $BIN_NAME..."
    cp -v downloads/*_linux_amd64 "setup-$BIN_NAME/"
